    ...
    return(
        <View className="h-[15%] p-3" style={{backgroundColor: props.backgroundColor ? props.backgroundColor : "white"}}>
            <View className="m-auto flex-row">
                <TouchableOpacity testID="checkoutBack" className="bg-blue-500 w-2/5 rounded-full h-3/4 m-2" onPress={props.backOnPress ? props.backOnPress : defaultBackOnPress}>
                    <AntDesign name="arrowleft" size={props.iconSize ? props.iconSize : 50} style={{margin: "auto"}}/>
                </TouchableOpacity>

                <TouchableOpacity testID="checkoutContinue" className="bg-blue-500 w-2/5 rounded-full h-3/4 m-2" onPress={props.grayedOut ? undefined : props.proceedOnPress} style={{opacity: props.grayedOut ? 0.50 : 1}}>
